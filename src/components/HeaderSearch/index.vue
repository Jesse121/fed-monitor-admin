<template>
  <div :class="{'show': show}" class="header-search" id="header-search">
    <svg-icon @click.stop="click" class="search-icon" name="search" />
    <el-select
      ...
      v-model="search"
    >
      <el-option :key="item.path" :label="item.meta.title.join(' > ')" :value="item" v-for="item in options" />
    </el-select>
  </div>
</template>
